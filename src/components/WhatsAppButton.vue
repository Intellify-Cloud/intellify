<template>
    <div class="pointer-events-none z-40">
        <button type="button" data-back-to-top
            class="back-to-top fixed left-1/2 bottom-6 inline-flex min-h-11 -translate-x-1/2 translate-y-3 items-center gap-2 rounded-full border border-stroke bg-surface px-4.5 py-2.5 text-sm font-extrabold text-fg opacity-0 pointer-events-none transition-[opacity,transform] duration-150 ease-out hover:bg-fg hover:text-fg">
        ...
        <a :href="`https://wa.me/${site.whatsapp}`" target="_blank" rel="noopener noreferrer"
            ...
            class="group pointer-events-auto fixed right-6 bottom-6 grid h-14 w-14 place-items-center">
            <span class="whatsapp-pulse"></span>
            <span class="whatsapp-ring"></span>
            <span class="whatsapp-core" aria-hidden="true">
...
.whatsapp-pulse {
    background: #25d366;
    opacity: 0.24;
    animation: whatsapp-pulse 2.1s infinite;
}

.whatsapp-ring {
    inset: -5px;
    border: 1px solid rgba(37, 211, 102, 0.42);
    animation: whatsapp-ring 2.1s infinite;
}
...
.whatsapp-button:hover .whatsapp-core {
